...
extern int mp[SIZE]; // Mark pointer. Peut être utilisé à divers usage :-)
extern int offset_mp; // Mark pointer. Peut être utilisé à divers usage :-)
// empile fp et positionne mp à cet endroit
#define SAVEFP stack[sp] = fp; mp[offset_mp++] = sp++;
// restore fp, dépile les arguments d'appels et empile la valeur de retour
#define RESTORE(n) sp = fp; fp = stack[fp]; stack[sp] = stack[sp+n+1]; sp++;
// changement de fp et appel de la fonction (sans arguement)
#define CALL(L) fp = mp[--offset_mp]; L();
